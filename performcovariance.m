...
% Vehicle Parameters and data from passed argument [m]
Rr   = vehicleparams.wheelRight/100;
Rl   = vehicleparams.wheelLeft/100;
b    = vehicleparams.track/100;
b    = vehicleparams.track;
n0   = res_encoder;
nl   = data.tick.Left;
nr   = data.tick.Right;

% Additional Vehicle parameters [m]
...
vehicleParameters.b = vehicleparams.track;
...
% Instance Cw 4x4 matrix 
Cw = [stdW^2 0 0 0;
      0 stdW^2 0 0;
      0 0 stdb^2 0;
      0 0 0 0];
% Perform suqare root fo previuos matrix
Sw=sqrt(Cw);

% % Calculate jacobian
% for i=1:1:length(nl)
%     J1 = [ pi*((nr(i))/n0)*cos(delta0), pi*((nr(i))/n0)*sin(delta0), 2*pi*((nr(i))/(n0*b));
...
for i=1:1:length(nl)-1
    ...
    y0     = v(i,2);
    delta0 = v(i,3);
    % calc odometric position
    x= data.pose.x(1)/100 + pi*((nr(i)*Rr+nl(i)*Rl)/n0)*cos(data.pose.psi(i));
    y= data.pose.y(1)/100 + pi*((nr(i)*Rr+nl(i)*Rl)/n0)*sin(data.pose.psi(i));
    delta= data.pose.psi(1) + 2*pi*((nr(i)*Rr-nl(i)*Rl)/(n0*b));
    v(i+1,:) = [x, y, delta];
    % calc phi
    ...
    CC(1+3*i:3+3*i,:)=Cx;
        ...
        P=0.95;
        vehiclePlotDD_simple( phi(i,:) , vehicleParameters, [] );
