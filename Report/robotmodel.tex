...
% \draw (4, 4) -- (4, 6);
 ...
 % Reference system 0
 	...
 	\draw [->] 	(A) -- (By) node[left]{\tiny $y$};
 	\draw [->] 	(A) -- (Bx) node[above]{\tiny $x$};
 % RF1
  	\coordinate [label = above right: \scriptsize $RF1$] (RF1) at (4,4);
 	\coordinate	(RF1x) at	($(RF1)+1*(0:1)$);
 	\coordinate	(RF1y)	at	($(RF1)+1*(90:1)$);
 	\fill(RF1) circle (1.4pt);
 	\draw [->] (RF1) -- (RF1y) node[left] {\tiny $y'$};
 	\draw [->] (RF1) -- (RF1x) node[above] {\tiny $x'$};
 % RFcam
 	\coordinate [label = below: \scriptsize $RF_{cam}$] (R) at (4.8,2.8);
 	\coordinate (Rx) at ($(R)+1*(22.5:1)$);
 	\coordinate (Ry) at ($(R)+1*(90+22.5:1)$);
 	\fill (R) circle (1.4pt);
	 \draw [->, darkgray] (R) -- (Ry) node[left] {\tiny $y_{cam}$};
 	 \draw [->, darkgray] (R) -- (Rx) node[above]{\tiny $x_{cam}$};
 \end{tikzpicture}